...
const TransportationForum = ({ selectedLocation, currentUser, onPostAdded }) => {
  ...
  const loadPosts = async () => {
    try {
      ...
      setPosts(allPosts.filter((p) => p.category === "transport"));
    } catch (err) {
  ...
  return (
    <div style={{ width: "100%", height: "100%", overflowY: "auto", padding: "1rem" }}>
      ...
      <h3>Existing Transportation Posts</h3>
      {posts.map((p) => (
...
export default TransportationForum;
