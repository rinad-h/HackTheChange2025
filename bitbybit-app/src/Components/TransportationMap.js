...
import API from "./apiController"; // to fetch posts
...
const TransportationMap = ({ selectedLocation, setSelectedLocation }) => {
  ...
  // Load transportation posts
  useEffect(() => {
    const loadPosts = async () => {
      try {
        const allPosts = await API.listPosts();
        setPosts(allPosts.filter((p) => p.category === "transport" && p.lat && p.lng));
      } catch (err) {
  ...
  const handleWardFeature = (feature, layer) => {
    layer.setStyle({ color: "#9b59b6", weight: 2, fillOpacity: 0.1 }); // purple
    layer.on({
