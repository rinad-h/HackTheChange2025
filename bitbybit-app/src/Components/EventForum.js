...
const EventForum = ({ selectedLocation, currentUser, onPostAdded }) => {
  ...
  const loadPosts = async () => {
    try {
      const allPosts = await API.listPosts("events"); // only events posts
      setPosts(allPosts);
    } catch (err) {
    ...
    <div style={{ width: "100%", height: "100%", overflowY: "auto", padding: "1rem" }}>
      ...
      {adding && (
        <form onSubmit={handleSubmit} style={styles.form}>
          ...
          <input
            ...
            value={selectedLocation ? `${selectedLocation.lat.toFixed(5)}, ${selectedLocation.lng.toFixed(5)}` : ""}
            readOnly
            style={styles.input}
          />
          <textarea placeholder="Description" value={desc} onChange={(e) => setDesc(e.target.value)} required style={styles.textarea} />
      ...
      {posts.map((p) => (
        <div key={p.id} style={styles.postBox}>
          ...
          <p>{p.desc}</p>
          <p><em>{p.author}</em> | {new Date(p.created_at).toLocaleString()}</p>
...
const styles = {
  addButton: { width: "100%", padding: "0.5rem", marginBottom: "0.5rem", cursor: "pointer", backgroundColor: "#99b3FF", border: "none", borderRadius: "6px", fontFamily: "'Elms Sans', sans-serif", color: "white", fontSize: "20px" },
